...
class DNN(nn.Module):
    def __init__(self,
        ...
        if DNN_layers2 > 1:
            self.dnn2.add_module('fc2_{}'.format(1), nn.Linear(DNN_hidden_dims1[-1], DNN_hidden_dims2[0]))
            ...
            self.dnn2.add_module('fc2_{}'.format(1), nn.Linear(DNN_hidden_dims1[-1], class_num))
        self.dnn2.add_module('Sigmoid', nn.Sigmoid())
